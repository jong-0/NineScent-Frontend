<template>
    <div>
        <h1>Checkout Page</h1>
        <div class="user-info">
            <h1 v-if="itemsData.length > 0">{{ itemsData[0].userName }}님께서 주문하려는 상품입니다.</h1>
            <div v-if="addressData && addressData.addrAddress">
                ...
                <button @click="goToAddressPage(itemsData[0].userId)">배송지 변경</button>
            </div>
        </div>
        <div v-if="itemsData && itemsData.length">
            <h2>주문 상품</h2>
            <div v-for="item in itemsData" :key="item.itemId" class="item-card">
                <h3>{{ item.itemName }}</h3>
                <p>Quantity: {{ item.quantity }}</p>
                <p>Price: {{ item.originalPrice }}</p>
                <p v-if="item.discountedPrice">Discounted Price: {{ item.discountedPrice }}</p>
        ...
        <div v-if="itemsData.length > 0" class="total-price">
            ...
            <p v-if="totalDiscount == 0">할인 금액: {{ totalDiscount }}원</p>
...
const totalOriginalPrice = computed(() => {
    return itemsData.value.reduce((sum, item) => sum + item.originalPrice * item.quantity, 0);
});

// 총 할인 금액 계산
const totalDiscount = computed(() => {
    return itemsData.value.reduce((sum, item) => {
        return sum + (item.discountedPrice ? item.originalPrice - item.discountedPrice : 0) * item.quantity;
    }, 0);
});

// 총 배송비 계산
const totalShippingFee = computed(() => {
    return itemsData.value.reduce((sum, item) => sum + item.shippingFee, 0);
});

// 최종 결제 금액 계산
const finalPrice = computed(() => {
    return totalOriginalPrice.value + totalShippingFee.value - totalDiscount.value;
});

// localStorage에서 orderItems 가져오기
const storedOrderItems = localStorage.getItem('orderItems');
const orderItems = storedOrderItems ? JSON.parse(storedOrderItems) : null;

const fetchOrderItems = async () => {
    if (orderItems) {
        itemsData.value = Array.isArray(orderItems) ? orderItems : [orderItems];

        const responseAddress = await addressApi.getAddress(itemsData.value[0].addressId);
        addressData.value = responseAddress;

        // orderItems 사용 후 localStorage 삭제 (더 이상 필요 없음)
        localStorage.removeItem('orderItems');
    } else {
        const responseItem = await checkoutApi.getPrepareOrder(userNo);
        itemsData.value = responseItem;
        const responseAddress = await addressApi.getAddress(responseItem[0].addressId);
        addressData.value = responseAddress;
    }
};
