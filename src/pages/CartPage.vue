    ...
    >
      <div class="d-flex align-items-center">
        <input
          ...
          class="me-2"
        />
        <label for="select-all-items" class="mb-0"
          >전체 선택</label
        >
      </div>
...
const isAllSelected = computed({
  get: () =>
    ...
    cartProducts.value.every((product) => product.selected),
  set: (value) => {
    cartProducts.value.forEach(
      (product) => (product.selected = value)
    );
  },
});
...
const selectedItemCount = computed(
  () =>
    cartProducts.value.filter((product) => product.selected)
      .length
);
...
const removeSelectedItems = async () => {
  const selectedItems = cartProducts.value.filter(
