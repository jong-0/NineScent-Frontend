        ...
        <div class="row gx-5">
            ...
            <div class="col-12 col-lg-8">
                <template v-if="orderType === 'cart' && cartData">
                    ...
                    <ul class="cart-item-list">
                        <li v-for="item in cartData" :key="item.itemId" class="cart-item">
                            <img :src="item.thumbSrc" alt="상품 이미지" class="product-image" />
                            <div class="item-details">
                                <p class="item-name">{{ item.title }}</p>
                                <p class="item-price">{{ item.price }}원 × {{ item.quantity }}개</p>
                            </div>
                        </li>
                    </ul>
                </template>

                <template v-else-if="orderType === 'item' && itemData">
                    <h6 class="mb-3">구매 상품</h6>
                    <div class="single-item">
                        <img :src="itemData.mainPhoto" alt="상품 이미지" class="product-image" />
                        <div class="item-details">
                            <p class="item-name">
                                {{ itemData.itemName }}
                            </p>
                            <p class="item-price">{{ itemData.price }}원 × {{ itemData.quantity }}개</p>
                        </div>
                    </div>
                </template>
            ...
            <div class="col-12 col-lg-4 mt-4 mt-lg-0">
                ...
                <div v-if="addressData" class="address-info">
                    ...
                    <button class="change-address-btn" @click="goToAddressPage(userNo)">배송지 변경</button>
                </div>
                <!-- 결제 요약 -->
                ...
                <button class="checkout-btn" @click="onPayment">결제하기</button>
            </div>
        </div>
    </div>
</template>
...
const fetchOrderData = () => {
    if (orderType === 'cart') {
        ...
        const storedShippingCost = sessionStorage.getItem('shippingCost');
        shippingCost.value = storedShippingCost ? Number(storedShippingCost) : 0;

        cartData.value = storedCart ? JSON.parse(storedCart) : null;
        ...
        itemData.value = storedItem ? JSON.parse(storedItem) : null;
    }
};
...
const fetchAddressData = async () => {
    // 기본 주소 불러오기 (새로운 주문 시)
    if (!userNo) return;
    const defaultResponse = await addressApi.getAddressDefault(userNo);
    ...
    if (addrNo) {
        try {
            ...
            console.error('선택한 주소를 불러오는 중 오류 발생:', error);
        }
...
const totalProductPrice = computed(() => {
    if (orderType === 'cart' && Array.isArray(cartData.value)) {
        return cartData.value.reduce((sum, item) => sum + (item.quantity || 1) * (item.price || 0), 0);
    } else if (orderType === 'item' && itemData.value) {
        return (itemData.value.price || 0) * (itemData.value.quantity || 1);
    }
    return 0;
});

// 총 결제 금액 (배송비 포함)
const finalTotalAmount = computed(() => totalProductPrice.value + shippingCost.value);
// 총 할인 금액
const totalDiscount = computed(() => {
    if (orderType === 'cart' && Array.isArray(cartData.value)) {
        return cartData.value.reduce((sum, item) => sum + item.discount * item.quantity, 0);
    } else if (orderType === 'item' && itemData.value) {
        return (itemData.value.discount || 0) * (itemData.value.quantity || 1);
    }
    return 0;
});
...
const onPayment = () => {
    ...
    const data = {
        ...
        buyer_tel: addressData.value?.addrContact || '010-0000-0000', // 구매자 전화번호
                    ...
                    : [
                          {
                              ...
                              discountedPrice: itemData.value.price - (itemData.value.discount || 0),
                          },
                      ],
